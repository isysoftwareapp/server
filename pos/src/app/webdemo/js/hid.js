...
async function testHIDDevices() {
  if (!navigator.hid) {
    showNotification(
      "HID Error",
      "WebHID is not supported in this browser. Please use a browser that supports WebHID, such as Google Chrome."
    );
    return;
  }
  ...
  try {
    ...
    if (devices.length === 0) {
      showNotification(
        "HID Error",
        "No HID devices selected. Please try again and select a device."
      );
      return;
    }
    ...
    showNotification(
      "HID Error",
      "Failed to access HID devices. Please check your permissions and try again."
    );
  }
}
...
async function connectToDevice(device) {
  try {
    if (device.opened) {
      showNotification(
        "HID Device",
        `${device.productName} is already connected`
      );
    } else {
      await device.open();
    ...
    if (error.name === "NotAllowedError") {
      ...
      errorMessage +=
        "Security error. This might be due to the page not being served over HTTPS.";
    } else {
...
function handleInputReport(event) {
  ...
  if (isBarcodeScannerData(dataArray)) {
    ...
    // Received unknown data from device - silently ignore
  }
}
...
function isBarcodeScannerData(dataArray) {
  ...
  // 检查起始字节和结束字节
  if (dataArray[0] !== 0x02 || dataArray[dataArray.length - 2] !== 0x03)
    return false;
